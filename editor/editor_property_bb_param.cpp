...
void EditorPropertyBBParam::update_property() {
	...
	if (param->get_value_source() == BBParam::BLACKBOARD_VAR) {
		...
		bottom_container->hide();
		BUTTON_SET_ICON(type_choice, get_editor_theme_icon(SNAME("LimboExtraVariable")));
	} else {
		...
		BUTTON_SET_ICON(type_choice, get_editor_theme_icon(Variant::get_type_name(param->get_type())));
	}
}
	...
	switch (p_what) {
		...
		case NOTIFICATION_THEME_CHANGED: {
...

			{
				...
				BUTTON_SET_ICON(type_choice, get_editor_theme_icon(type));
			}
